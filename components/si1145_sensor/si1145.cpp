...
void SI1145Component::update() {
  if (this->measurement_in_progress_) {
    ESP_LOGV(TAG, "Measurement already in progress, skipping update cycle");
    return;
  }

  // Start a forced measurement and collect result asynchronously.
  this->measurement_in_progress_ = true;
  write8_(SI1145_REG_COMMAND, SI1145_ALS_FORCE);
  this->schedule_measurement_read_(0);
}

void SI1145Component::schedule_measurement_read_(uint32_t elapsed_ms) {
  uint8_t irq_status = read8_(SI1145_REG_IRQSTAT);
  bool data_ready = (irq_status & SI1145_REG_IRQSTAT_ALS) != 0;
  bool timed_out = elapsed_ms >= 40;

  if (data_ready || timed_out) {
    this->read_and_publish_(irq_status, timed_out);
    this->measurement_in_progress_ = false;
    return;
  }

  this->set_timeout("measurement-read", 2, [this, elapsed_ms]() { this->schedule_measurement_read_(elapsed_ms + 2); });
}

void SI1145Component::read_and_publish_(uint8_t irq_status, bool timed_out) {
  if (timed_out) {
    ESP_LOGW(TAG, "Measurement not ready after timeout, using latest available sample");
  }

  const uint16_t visible_raw = read_visible_raw_();
  const uint16_t infrared_raw = read_infrared_raw_();
...
uint16_t SI1145Component::read_visible_raw_() { return read16_(SI1145_REG_ALSVISDATA0); }
...
uint16_t SI1145Component::read_infrared_raw_() { return read16_(SI1145_REG_ALSIRDATA0); }
...
void SI1145Component::reset_() {
  ...
  uint8_t response_before_reset = read8_(SI1145_REG_RESPONSE);
  write8_(SI1145_REG_COMMAND, SI1145_RESET);
  if (!this->wait_for_response_change_(response_before_reset, 25)) {
    ESP_LOGW(TAG, "Reset response timeout");
  }

  uint8_t response_before_hwkey = read8_(SI1145_REG_RESPONSE);
  write8_(SI1145_REG_HWKEY, 0x17);
  if (!this->wait_for_response_change_(response_before_hwkey, 25)) {
    ESP_LOGW(TAG, "HWKEY response timeout");
  }
}
...
bool SI1145Component::wait_for_response_change_(uint8_t previous_response, uint32_t timeout_ms) {
  uint32_t start = millis();
  while ((millis() - start) < timeout_ms) {
    if (read8_(SI1145_REG_RESPONSE) != previous_response) {
      return true;
    }
    yield();
  }
  return read8_(SI1145_REG_RESPONSE) != previous_response;
}
