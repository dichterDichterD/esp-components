...
class SI1145Component : public PollingComponent, public i2c::I2CDevice {
 ...
 protected:
  // Continue measurement without blocking the main loop.
  void schedule_measurement_read_(uint32_t elapsed_ms);
  void read_and_publish_(uint8_t irq_status, bool timed_out);
  bool wait_for_response_change_(uint8_t previous_response, uint32_t timeout_ms);
